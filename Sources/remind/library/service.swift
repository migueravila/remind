...
class ReminderService {
        ...
        for calendar in calendars {
            ...
            lists.append(
                ReminderList(
                    ...
                    color: calendar.cgColor?.components?.description, reminderCount: reminderCount))
        }
    ...
    func createList(name: String) async throws -> ReminderList {
        ...
        return ReminderList(id: calendar.calendarIdentifier, title: calendar.title, reminderCount: 0)
    }
        ...
        return await withCheckedContinuation { continuation in
            ...
            eventStore.fetchReminders(matching: predicate) { ekReminders in
                let reminders = (ekReminders ?? []).map { ekReminder in
                    ReminderItem(
                        id: ekReminder.calendarItemIdentifier, title: ekReminder.title ?? "", notes: ekReminder.notes,
                        isCompleted: ekReminder.isCompleted,
                        priority: ReminderItem.Priority(rawValue: ekReminder.priority) ?? .none,
                        dueDate: ekReminder.dueDateComponents?.date, listName: ekReminder.calendar.title)
                }
    ...
    func createReminder(_ reminder: ReminderItem, in listName: String) async throws {
        ...
        guard let calendar = calendars.first else { throw RemindError.listNotFound }
